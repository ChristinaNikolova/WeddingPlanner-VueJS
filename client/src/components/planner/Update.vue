...
export default {
  ...
  methods: {
    onCancelFormHandler() {
      this.$router.push({ path: '/plan', params: { id: this.id } });
    },
    async onSubmitHandler() {
      ...
      await plannersService
        .update(this.id, this.data.description, this.data.date, this.data.budget, this.data.location, this.data.bride, this.brideId, this.data.groom, this.groomId)
        .then((res) => {
          if (res.message) {
